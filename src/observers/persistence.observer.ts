...
function isFromBlockedContact(blockedContacts: BlockedContactRepository, jid: string, participant?: string | null): Promise<boolean> {
...
export class PersistenceObserver extends TypedEventEmitter<PersistenceObserverEvents> {
    constructor(
        ...
        client.on('message.received', async (message) => {
            if (!message.fromMe && (await isFromBlockedContact(this.blockedContacts, message.remoteJid, message.participant))) {
                return;
            }
        ...
        client.on('message.deleted', (targetId) => {
            void this.repository.markDeleted(targetId);
        });

        client.on('message.edited', (targetId, newText) => {
            void this.repository.markEdited(targetId, newText);
        });
        ...
        client.on('message.status-changed', (targetId, status) => {
            ...
            void this.repository.updateStatus(targetId, status);
        });
    }
}
